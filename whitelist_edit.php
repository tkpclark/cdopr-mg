...
<td>单个添加</td>
<td>
<form name=blklist_edit_form action=whitelist_edit_do.php method=post >
...
<td>批量添加</td>
<td>
<form ENCTYPE="multipart/form-data" ACTION="whitelist_edit.php" METHOD="POST">
    ...
    if(move_uploaded_file($temp_name, $upload_dir.$file_name)){
		...
		while (!feof($file_handle)) {
			...
			if(1)
			{
				...
				//echo "格式正确!<br>";
				$sql=sprintf("insert into wraith_whitelist(phone_number,time) values('%s',now())",$line);
				//echo $sql;
				exsql($sql);
				$count++;
			}
